...
#define ECT_SII_START 0x0040
    ...
    enum
    {
        ...
        ECT_REG_DLSTAT = 0x0110,		// 	ESC数据链路状态
		// application layer
        ECT_REG_ALCTL = 0x0120, 		//	AL状态控制位见pdf61页
        ECT_REG_ALSTAT = 0x0130,		// 	AL当前状态pdf61页
        ECT_REG_ALSTATCODE = 0x0134,	//	AL状态码：显式应用层状态
		...
		// EEPROM 见pdf 73页
        ECT_REG_EEPCFG = 0x0500,		//	EEPROM控制
        ECT_REG_EEPCTL = 0x0502,		//	EEPROM控制和状态寄存器
        ECT_REG_EEPSTAT = 0x0502,
        ECT_REG_EEPADR = 0x0504,		//	请求操作的EEPROM地址
		...
		// 分布时钟相关寄存器 见pdf81页
        ECT_REG_DCTIME0 = 0x0900,		//	端口0接收时刻
        ECT_REG_DCTIME1 = 0x0904,		//	端口1接收时刻
        ECT_REG_DCTIME2 = 0x0908,		//	端口2接收时刻
        ECT_REG_DCTIME3 = 0x090C,		//	端口3接收时刻
        ECT_REG_DCSYSTIME = 0x0910,		//	本地系统时间
        ECT_REG_DCSOF = 0x0918,			//	数据帧处理单元接收时间
        ECT_REG_DCSYSOFFSET = 0x0920,	//	时间偏移
        ECT_REG_DCSYSDELAY = 0x0928,	//	传输延时
        ECT_REG_DCSYSDIFF = 0x092C,		//	系统时间差：4byte bit31表示哪一个时间快
        ECT_REG_DCSPEEDCNT = 0x0930,	//	调节本地系统时间的带宽
        ECT_REG_DCTIMEFILT = 0x0934,	//	过滤深度：系统时间偏差计算的平均次数
        
		ECT_REG_DCCUC = 0x0980,
        ECT_REG_DCSYNCACT = 0x0981,		//	
        ECT_REG_DCSTART0 = 0x0990,		//	周期运行开始时间
        ECT_REG_DCCYCLE0 = 0x09A0,		//	两个连续SYNC0脉冲直接的时间(ns)
        ECT_REG_DCCYCLE1 = 0x09A4		//	SYNC0脉冲和SYNC1脉冲之间的时间(ns)
    };
