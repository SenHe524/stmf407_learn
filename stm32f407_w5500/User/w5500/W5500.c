...
unsigned char Phy_Addr[6] = {0x00,0x08,0xDC,0x01,0x02,0x03};	//物理地址(MAC) 
unsigned char IP_Addr[4] = {192,168,1,199};	//本机IP地址 
unsigned char S0_Port[2] = {0x1E,0x40};	//端口0的端口号(5000) 

unsigned char S0_DIP[4] = {192,168,1,190};		//端口0目的IP地址 
unsigned char S0_DPort[2] = {0x1F,0x40};	//端口0目的端口号(6000) 
...
unsigned char S0_Mode =TCP_SERVER;	//端口0的运行模式,0:TCP服务器模式,1:TCP客户端模式,2:UDP(广播)模式
/***************----- 端口的运行状态 -----***************/
...
//	GPIO_PinAFConfig(W5500_SPI3_SCS_PORT, GPIO_PinSource4, GPIO_AF_SPI3);
	...
	GPIO_Init(W5500_SPI3_PORT, &GPIO_InitStructure);
	/* 初始化SCS引脚 */
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;// 引脚复用
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;// 推挽输出
...
* 说明    : 无
*******************************************************************************/
unsigned char SPI3_Send_Byte(unsigned char dat)
{
...
{
	GPIO_ResetBits(W5500_SPI3_SCS_PORT, W5500_SPI3_SCS);//置W5500的SCS为低电平
	...
	GPIO_SetBits(W5500_SPI3_SCS_PORT, W5500_SPI3_SCS); //置W5500的SCS为高电平
}
...
unsigned short Read_SOCK_Data_Buffer(SOCKET s, unsigned char *dat_ptr)
{
	...
	j=SPI3_Send_Byte(VDM|RWB_READ|(s*0x20+0x18));//写控制字节,N个字节数据长度,读数据,选择端口s的寄存器
...
//	j = Read_W5500_1Byte(RTR);
//	j = Read_W5500_1Byte(RTR+1);
	//设置重试次数，默认为8次 
	//如果重发的次数超过设定值,则产生超时中断(相关的端口中断寄存器中的Sn_IR 超时位(TIMEOUT)置“1”)
	Write_W5500_1Byte(RCR,8);
////	j = Read_W5500_1Byte(RCR);
...
////	j = Read_W5500_1Byte(IMR);
//	Write_W5500_1Byte(SIMR,S0_IMR);// Socket 0 中断使能
////	j = Read_W5500_1Byte(SIMR);
//	Write_W5500_SOCK_1Byte(0, Sn_IMR, IMR_SENDOK | IMR_TIMEOUT | IMR_RECV | IMR_DISCON | IMR_CON);
////	j = Read_W5500_1Byte(Sn_IMR);
}
...
void Socket_Init(SOCKET s)
{
	...
	switch(s)
	{
		case 0:
			...
			Write_W5500_SOCK_1Byte(0, Sn_DPORTR, S0_DPort[0]*256+S0_DPort[1]);
			Write_W5500_nByte(Sn_DIPR,S0_DIP,4);
			j = Read_W5500_1Byte(Sn_DIPR);
			j = Read_W5500_1Byte(Sn_DIPR+1);
			j = Read_W5500_1Byte(Sn_DIPR+2);
			j = Read_W5500_1Byte(Sn_DIPR+3);
			j = Read_W5500_SOCK_1Byte(0,Sn_PORT);
			j = Read_W5500_SOCK_1Byte(0,Sn_PORT+1);
			j = Read_W5500_SOCK_1Byte(0,Sn_DPORTR);
			j = Read_W5500_SOCK_1Byte(0,Sn_DPORTR+1);
			j = 0x11;
		(void) j;
			break;
...
//	j = Read_W5500_SOCK_1Byte(s,Sn_SR);
	(void) j;
